...
app.post("/api/create-pix", async (req, res) => {
  try {
    ...
    if (!Number.isFinite(value) || value < 1 || value > 10000) {
      return res.status(400).json({ error: "Valor inválido. Use entre R$ 1 e R$ 10.000." });
    }
    ...
    if (!process.env.MP_ACCESS_TOKEN) {
      return res.status(500).json({ error: "Mercado Pago ainda não está configurado no servidor." });
    }
    ...
    donation.status = data.status || payment?.status || "pending";
    ...
    console.error(error);
    res.status(500).json({ error: "Erro interno ao criar o Pix." });
  }
});

app.post("/api/webhook/mercadopago", async (req, res) => {
  // Respond quickly as recommended by Mercado Pago.
  res.sendStatus(200);

  try {
    const orderId = req.body?.data?.id;
    if (!orderId || !process.env.MP_ACCESS_TOKEN) return;
    ...
    const donation = donations.get(String(orderId));

    if (!donation) return;

    donation.status = order.status || payment?.status || "pending";
    ...
    if (approved && !donation.alertSent) {
...
app.get("/api/status/:orderId", (req, res) => {
  const donation = donations.get(String(req.params.orderId));
...
app.get("/overlay", (req, res) => {
  res.sendFile(require("path").join(__dirname, "public", "overlay.html"));
});

app.listen(PORT, HOST, () => {
  console.log(`KevenZL Pix Alertas rodando em http://${HOST}:${PORT}`);
});
